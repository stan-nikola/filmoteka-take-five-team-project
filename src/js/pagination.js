...
export async function renderMovies2() {
  ...
  const tempCountPage = 8; //временная переменная - нужно заменить на dataMovies.total_pages

  pagination(tempCountPage, movieInfo);
}
...
function pagination(total_pages, dataRowArr) {
  ...
  function createPaginationEl(page) {
    ...
    paginationEl.addEventListener('click', async () => {
      ...
      const dataMovies2 = await fetchHomeTrendingMovies3();
